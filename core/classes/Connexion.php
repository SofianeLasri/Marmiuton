...
// fonction de connexion
    ...
    {
        require "core/conf/ConfigurationGenerale.php";
        $h = $bddHost;
        $port = $bddPort;
        $d = $bddName;
        $l = $bddUsername;
        ...
        $t = self::$tabUTF8;
        try {
            self::$pdo = new PDO("mysql:host=$h;port=$port;dbname=$d;charset=utf8", $l, $p, $t);
            self::$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        } catch (PDOException $e) {
            echo "Erreur de connexion !";
            error_log("Connection failed: " . $e->getMessage());
            error_log("dsn: mysql:host=$h;port=$port;dbname=$d;charset=utf8");
        }
    }
}